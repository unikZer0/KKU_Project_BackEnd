<x-admin-layout>
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div id="dashboard-chart" class="lg:col-span-7 bg-white rounded-lg border p-4"
            data-total-requests='@json($borrowStatusMonthly['TotalRequests'] ?? [])' data-approved='@json($borrowStatusMonthly['Approved'] ?? [])'
            data-rejected='@json($borrowStatusMonthly['Rejected'] ?? [])' data-months='@json($borrowStatusMonthly['months'] ?? [])'
            data-selected-year='@json($selectedYear)' data-available-years='@json($availableYears)'>
            <canvas></canvas>
        </div>
        ...
        <div id="recent-activities" data-requests='@json($recentRequests)'
